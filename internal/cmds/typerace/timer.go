...
func promptUser(inputChan, timeChan chan string) {
	input := ""
	timer := "0.000"
	fmt.Print("\r", colors.Blue+"Start typing: "+colors.Reset, input, "█ ", timer, "secs")
	for {
		...
		case timer = <-timeChan:
			fmt.Print("\r", colors.Blue+"Start typing: "+colors.Reset, input, "█ ", colors.Red+timer+colors.Reset, "secs"+colors.Reset)
		default:
		}
	}
}
